...
export default function ControleUsoFrota() {
  ...
  const fetchData = async (force = false) => {
    ...
    if (contractsRes.data) {
      ...
      const getMedicaoType = (equipId: string, dataStr: string): "horas" | "diarias" => {
        const d = new Date(dataStr + "T00:00:00").getTime();
        let tipoFinal: "horas" | "diarias" = "horas";
        let maxDataEntrega = 0;

        // Check single equipment contracts
        const ceSingle = [...contractsRes.data].filter(c => c.equipamento_id === equipId);
        ceSingle.forEach(c => {
          tipoFinal = (c.tipo_medicao || "horas") as "horas" | "diarias";
        });

        // Check contratos_equipamentos
        if (ceRes.data) {
          ceRes.data.forEach((ce: any) => {
            if (ce.equipamento_id === equipId) {
              const entrega = ce.data_entrega ? new Date(ce.data_entrega + "T00:00:00").getTime() : 0;
              const devolucao = ce.data_devolucao ? new Date(ce.data_devolucao + "T00:00:00").getTime() : Infinity;
              if (d >= entrega && d <= devolucao && entrega >= maxDataEntrega) {
                maxDataEntrega = entrega;
                const tipo = contractMedicaoTypes.get(ce.contrato_id);
                if (tipo) tipoFinal = tipo;
              }
            }
          });
        }

        // Check aditivos_equipamentos
        if (aeRes.data) {
          aeRes.data.forEach((ae: any) => {
            if (ae.equipamento_id === equipId) {
              const entrega = ae.data_entrega ? new Date(ae.data_entrega + "T00:00:00").getTime() : 0;
              const devolucao = ae.data_devolucao ? new Date(ae.data_devolucao + "T00:00:00").getTime() : Infinity;
              if (d >= entrega && d <= devolucao && entrega >= maxDataEntrega) {
                maxDataEntrega = entrega;
                const contratoId = aditivoToContract.get(ae.aditivo_id);
                if (contratoId) {
      ...
      // We will keep this function around just in case, but we won't call setRawContractsData 
      // because it's not defined in the state.
    }
  ...
  const openNew = () => {
    ...
    const isDiariaDefault = defaultEquip ? equipMedicaoTypes.get(defaultEquip) === "diarias" : false;
    setEditingId(null);
    setForm({
