...
export const FaturamentoContent = () => {
  ...
  const fetchMedicoesEGastos = useCallback(async (contratoId: string, inicio: string, fim: string, checkActive?: () => boolean) => {
    ...
    const [equipRes, gastosRes, ajustesRes] = await Promise.all([
      ...
      supabase.from("gastos").select("id, descricao, tipo, valor, data, equipamento_id, classificacao:status").in("equipamento_id", allEquipIdsWithAditivos).gte("data", inicio).lte("data", fim).order("data", { ascending: false }),
    ...
    const newEquipForms: EquipFormItem[] = filteredEquipIds.map(eqId => {
      ...
      let horasContratadas = ajuste && ajuste.horas_contratadas !== null && ajuste.horas_contratadas !== undefined ? Number(ajuste.horas_contratadas) : baseHorasContratadas;
      let horaMinima = ajuste && ajuste.hora_minima !== null && ajuste.hora_minima !== undefined ? Number(ajuste.hora_minima) : baseHoraMinima;
      ...
      return {
        ...
        cobranca_parcial: "proporcional_minimo" as const,
    ...
    newEquipForms.forEach(ef => {
      ...
      const isProporcional = ef.primeiro_mes || ef.proporcional_devolucao;
      const horasMedidasCalculo = Math.max(0, ef.horas_medidas - ef.horas_indisponiveis);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        conta_bancaria_id: !formContaBancariaId || formContaBancariaId === "none" ? null : formContaBancariaId,
        empresa_faturamento_id: !formEmpresaFaturamentoId || formEmpresaFaturamentoId === "none" ? null : formEmpresaFaturamentoId,
      } as any;
